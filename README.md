...
### 1.1. Смешение основной нумерации и приложений
В оглавлении (стр. 7–8) внутри главы 2 появляются пункты **2.9, 2.10, 2.11, 2.12, 2.13, 2.14, 2.15, 2.16, 2.17**, которые на самом деле относятся к приложениям (блок-схемы, документация, примеры деталей, требования к оборудованию).
**Почему это ошибка:** Приложения (Приложение А, Б, В) должны нумероваться отдельно и не входить в нумерацию глав. Их заголовки в тексте («Приложение А. Документация…») не соответствуют оглавлению.
**Исправление:** Убрать из оглавления пункты 2.9–2.17. Вместо этого в оглавлении должны быть строки:
```
Приложение А. Документация по установке, настройке и эксплуатации ... 58
Приложение Б. Примеры работы модуля main.exe ... 62
Приложение В. Требования к оборудованию ... 65
```
Внутри самих приложений не должно быть подзаголовков с цифрами 2.9, 2.10 и т.д. – используйте буквенно-цифровую нумерацию (А.1, А.2) или просто абзацы.
...
### 1.3. Неправильная нумерация глав
Глава 2 называется «Реализация и экспериментальные исследования», но внутри неё идёт подраздел «2.8. Обсуждение ограничений…», затем **Заключение** (без номера), затем приложения. Заключение должно быть отдельным разделом (не внутри главы 2). В оглавлении заключение вынесено правильно (после 2.8), но в тексте оно начинается на стр. 56 без явного отделения от главы 2. Лучше добавить разрыв страницы.
...
### 1.7. Оглавление ссылается на страницы, которые могут не совпадать после вставки рисунков
Сейчас указаны страницы 18, 20, 22 и т.д. После добавления рисунков и исправления структуры нумерация изменится. Нужно будет обновить оглавление автоматически.
